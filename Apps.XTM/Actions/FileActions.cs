...
public class FileActions(InvocationContext invocationContext, IFileManagementClient fileManagementClient) : XtmInvocable(invocationContext)
{
        ...
        [ActionParameter] DownloadTranslatedInteroperableFileRequest input)
    {
        ...
        var generated = await GenerateFiles(project, new GenerateFileRequest
        {
            FileType = "TARGET",
            JobIds = [input.JobId],
        });
        var file = generated.Files.SingleOrDefault(x => x.JobId == input.JobId && x.FileType == "TARGET");
        ...
        return new(await _fileManagementClient.UploadAsync(
        ...
        [ActionParameter] UploadSelectedSourceXliffRequest input)
    {
        ...
        var prepared = XliffSourceSelection.Prepare(sourceBytes, input.ExcludeSegmentStates,
            input.File.Name ?? fileName, input.File.ContentType, projectDetails.SourceLanguage);
